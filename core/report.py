...
class PDFReport(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, "MARS — Malware Analysis & Reverse-engineering System", border=False, ln=True, align="C")
...
class ReportGenerator:
    ...
    def _build_pdf(self, data, output_path):
        ...
        def section_header(pdf_obj, number, title):
            ...
            pdf_obj.cell(0, 9, f"  {number}. {title}", ln=True, fill=True)
        ...
        def subsection_header(pdf_obj, title, color=(0, 50, 150)):
            ...
            pdf_obj.cell(0, 8, f"▸  {title}", ln=True)
            pdf_obj.set_text_color(0, 0, 0)
        ...
        def kv_row(pdf_obj, key, value, col_width=65):
            ...
            pdf_obj.cell(col_width, 6, f"  {clean_key}:", ln=False)
            pdf_obj.set_font("helvetica", "", 10)
            pdf_obj.multi_cell(0, 6, clean_val)
            pdf_obj.x = pdf_obj.l_margin
            pdf_obj.set_text_color(0, 0, 0)
        ...
        if not dynamic_res:
            pdf.set_font("helvetica", "I", 10)
            pdf.cell(0, 6, "  No dynamic analysis performed.", ln=True)
        else:
            for target_exe, telemetry in dynamic_res.items():
                ...
                for cat in TELEMETRY_KEYS:
                    ...
                    pdf.set_fill_color(28, 38, 60) if row_fill else pdf.set_fill_color(22, 30, 50)
                    ...
                    pdf.set_text_color(255, 200, 80) if notable else pdf.set_text_color(100, 100, 100)
                ...
                for cat in TELEMETRY_KEYS:
                    ...
                    pdf.cell(0, 7, f"  {label}  ({len(events)} events)", ln=True, fill=True)
                    pdf.set_text_color(0, 0, 0)
                    pdf.set_font("helvetica", "", 8)

                    for ev in events:
                        clean = str(ev).encode("latin-1", "replace").decode("latin-1")
                        # Highlight network / persistence / process-spawn events
                        upper = clean.upper()
